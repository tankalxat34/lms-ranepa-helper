...
const chromeStorageKeyLinks = "helper-my-lms-links";
...
var Services = {
  ...
  urlshorter: {
    create: function () {
      fetch(
        "https://clck.ru/--?json=true&url=" +
          encodeURIComponent(document.querySelector("#helper-urlshorter_input").value)
      )
        .then((res) => res.json())
        .then((json) => {
  ...
  rugost: {
    ...
    s0: function () {
      ...
      textarea_result.value = `${author.value.split(",")[0].trim()} ${title.value} / ${
        author.value
      }. — ${num_redaction.value}. — ${city.value} : ${publisher.value}, ${year.value}. — ${
        pages_count.value
    ...
    s1: function () {
      ...
      textarea_result.value = `${author.value.split(",")[0].trim()} ${title.value} / ${
        author.value
      }. — Текст : непосредственный // ${magazine_title.value}. — ${year.value}. — № ${
        magazine_number.value
      }. — С. ${page.value}.`;
    },
    /**
     * Статьи из сборника
    ...
    s2: function () {
      ...
      textarea_result.value = `${author.value.split(",")[0].trim()} ${title.value} / ${
        author.value
      }. — Текст : непосредственный // ${sbornik_title.value}. — ${city.value} : ${
        publisher.value
      }, ${year.value}. — С. ${page.value}.`;
    },
    /**
     * Интернет ресурс
     */
    s4: function () {
      ...
      if (author.value) {
        textarea_result.value = `${author.value.split(",")[0].trim()} ${title.value} / ${
          author.value
        }. — Текст : электронный // ${web_title.value} : [сайт]. — URL: ${
          web_url.value
        } (дата обращения: ${date.toLocaleDateString()}).`;
      } else {
        textarea_result.value = `${title.value}. — Текст : электронный // ${
          web_title.value
        } : [сайт]. — URL: ${web_url.value} (дата обращения: ${date.toLocaleDateString()}).`;
      }
    },
  },
};
...
function addListenersToServices() {
   ...
   */
  document
    .querySelector("#helper-urlshorter_create")
    .addEventListener("click", Services.urlshorter.create);
  document
    .querySelector("#helper-urlshorter_remove")
    .addEventListener("click", Services.urlshorter.clear);
...
fetchDoc("nodes/my/mainBlock.html")
      ...
      .then((json) => {
        if (json[0].name !== MANIFEST.version) {
          extentionNode.innerHTML = `<details>
            <summary><b style="color: #fd7e14">Доступна версия <a href="${
              json[0].html_url
            }" target="_blank">${json[0].name}</a></b></summary>            
                ${markdown(json[0].body)}
                <a href="${
                  json[0].zipball_url
                }" class="btn btn-primary"><i class="fa fa-download"></i> Скачать v${
            json[0].name
          }</a>
            </details>`.replaceAll("\r\n", "<br>");
        } else {
  ...
  .then(() => {
    fetchDoc("nodes/my/servicesBlock.html")
      .then((text) => {
        let extentionNode = document.createElement("section");
      ...
      .then(async () => {
        ...
        // получение и вывод сохраненных ссылок на главную страницу
        const place = document.querySelector("#helper-my-lms-links-place");
        /**
         * @type object
         */
        const storage = await chrome.storage.sync.get(chromeStorageKeyLinks);

        /**
         * @type object[]
         */
        const storageLinks = storage[chromeStorageKey] || [];
        // console.log(storageLinks);
        storageLinks.map((link, index) => {
          const li = new HTMLComp(
            `
            <li><a href="<% href %>" data-id="<% id %>"><% courseTitle %></a></li>
          `,
            { ...link }
          );
          li.html.addEventListener("contextmenu", async (e) => {
            e.preventDefault();
            ...
            chrome.storage.sync.set(
              {
                [chromeStorageKey]: [...storageLinks].filter(
      ...
      .then(() => {
        ...
        chrome.storage.sync.get(["_option_names_array"], (options) => {
          ...
          chrome.storage.sync.get(_opt_names, (options) => {
            ...
            if (disabled_counter_services === max_counter_services)
              document.querySelector("#helper-services").hidden = true;

            document
              .querySelector('[data-block="mydashboard"]')
              .before(document.querySelector('[data-block="calendar_upcoming"]'));
            let h5_node_1 = document.querySelector('[data-block="calendar_upcoming"] > div > h5');
            if (
              document.querySelector('[data-block="calendar_upcoming"] > div > div > div > div')
                .childElementCount - 1
            ) {
              document.querySelector(
                '[data-block="calendar_upcoming"]'
              ).innerHTML = `<details class="m-3">
                    <summary>
            ...
            if (options["helper-settings-show_courses_filters"]) {
              ...
              a_filter_only_not100.addEventListener("click", () => {
                ...
                for (let filter_element of document.querySelectorAll(
                  "div.filter-btn-box .filter-btn"
                )) {
                  filter_element.classList.remove("active");
                }

                a_filter_only_not100.classList.add("active");
                let cards = document.querySelector('div[data-region="paged-content-page"]')
                  .children[0].childNodes;

                for (let card of cards) {
                  let card_text = card.textContent.replaceAll("\n", " ").trim().toLowerCase();
                  if (
                    regex_findall(/прогресс: *100\%/gm, card_text)[0] ||
                    card_text.includes("100%")
                  ) {
                    card.hidden = true;
                  } else {
                    card.hidden = false;
              ...
              document
                .querySelector(".mydashboard-filters-sort-display-box")
            ...
            if (options["helper-settings-show_searchinput_courses"]) {
              ...
              input_search.placeholder =
                "Начните вводить название курса/филиал/фио преподавателя для поиска...";
