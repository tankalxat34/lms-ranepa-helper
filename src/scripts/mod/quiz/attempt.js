...
function addListenersToChat() {
  ...
  document.querySelector("#helper-chatgpt_input").addEventListener("input", () => {
    ...
    document.querySelector("#helper-chatgpt_input").style.height =
      document.querySelector("#helper-chatgpt_input").scrollHeight + "px";
  });
  ...
  document.querySelector("#helper-btn-chatgpt_export").addEventListener("click", () => {
    downloadFileFromText(
      ...
      `<!-- Generated automatically using the LMS RANEPA HELPER extension (c) tankalxat34 -->
<style type="text/css">
* {
...
<h3>${document.querySelector(".page-header-headings > h1").textContent}</h3>` +
        document.querySelector("#helper-chatgpt_response").innerHTML
    );
  });
    ...
    .then(() => {
      ...
      chrome.storage.sync.get(
        [
          ...
          "chatgpt_user_object",
          "helper-chatgpt-model",
          "helper-chatgpt-access_token",
          "helper-settings-get_localuser_token",
        ],
        (options) => {
          var _opt_names = options["_option_names_array"];
          var chatgpt_user_object = JSON.parse(
            options["chatgpt_user_object"] ? options["chatgpt_user_object"] : "{}"
          );
          ...
          if (options["helper-settings-get_localuser_token"])
            chatgpt_user_object.accessToken = options["helper-chatgpt-access_token"];
          ...
          chrome.storage.sync.get(_opt_names, (options) => {
            ...
            if (options["helper-settings-show_chatgpt"]) {
              fillChatGPTButtons(
                options["helper-chatgpt-model"] || "gpt-3.5-turbo",
                chatgpt_user_object
              );
              addListenersToChat();
            ...
            if (options["helper-settings-changeable_form_action"]) {
              ...
              let form_action_input = document.querySelector(
                "#helper-settings-changeable_form_action-input"
              );
              ...
              form_action_input.addEventListener("keyup", () => {
                ...
                if (form_action_input.value === "")
                  document.querySelector("#responseform").action = default_formaction;
              });
            }
          });
        }
