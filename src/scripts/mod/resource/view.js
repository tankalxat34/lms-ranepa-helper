    ...
    chrome.storage.sync.get(_opt_names, (options) => {
        window.onload = () => {
            ...
            if (document.querySelector("source") && document.querySelector("source").type.split("/")[0] === "video") {
                ...
                a_download_video.innerHTML = `<a href="${document.querySelector("source").src}" class="badge badge-info rounded mb-1" download><i class="fa fa-download"></i> Скачать: <span class="font-weight-normal">${document.querySelector("source").src.split(".")[document.querySelector("source").src.split(".").length - 1]}</span></a> `
                a_download_video.innerHTML += `<a href="${document.querySelector("source").src}" target="_blank" class="badge badge-info rounded mb-1"><i class="fa fa-external-link"></i> Открыть: <span class="font-weight-normal">во вкладке</span></a>`
